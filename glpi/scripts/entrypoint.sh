...
default_glpi_conf="/usr/share/glpi"
...
print_help() {
    ...
    echo "  PHP_INI_SCAN_DIR: additional .ini directories (default: /etc/php/conf.d)."
    echo "  PHP_FPM_CONF_DIR: PHP-FPM configuration directory (default: /etc/php-fpm)."
    echo "  GLPI_APP_DIR: GLPI application directory (default: /var/www/html)."
...
init_dir_app(){
    ...
    tar -xf $source_dir/glpi-*.tgz -C $dest_dir --strip-components=1
    init_dir_cp_file $source_dir/downstream.php $dest_dir/inc
    chmod -R 750 $dest_dir
}
...
init_dir_if_empty(){
    ...
    && init_dir_default "$default_php_conf" "$PHP_CONF_DIR" \
    ...
    && init_dir_default "$default_php_fpm_conf" "$PHP_FPM_CONF_DIR" \
    || print_info "PHP_FPM_CONF_DIR: \"$PHP_FPM_CONF_DIR\" [OK]."
    
    check_dir_is_empty "$GLPI_APP_DIR"
    [[ -n "$dir_is_empty" ]] \
    && print_info "GLPI_APP_DIR: \"$GLPI_APP_DIR\" empty, creating..." \
    && init_dir_app "$default_glpi_conf" "$GLPI_APP_DIR" \
    || print_info "GLPI_APP_DIR: \"$GLPI_APP_DIR\" [OK]."
    
    check_dir_is_empty "$GLPI_CONF_DIR"
    [[ -n "$dir_is_empty" ]] \
    && print_info "GLPI_CONF_DIR: \"$GLPI_CONF_DIR\" empty, creating..." \
    && init_dir_cp_file "$default_glpi_conf/local_define.php" "$GLPI_CONF_DIR" \
    ...
    && init_dir_app_data "$GLPI_DATA_DIR" \
    || print_info "GLPI_DATA_DIR: \"$GLPI_DATA_DIR\" [OK]."
}

mode_inspector(){
    print_info "MODO DE INSPEÇÃO."
...
mode_copy(){
    ...
    init_dir_cp_dir $default_php_conf $1

    print_info_short "  - $default_php_fpm_conf"
    init_dir_cp_dir $default_php_fpm_conf $1

    print_info_short "  - $default_glpi_conf"
    init_dir_cp_dir $default_glpi_conf $1
}
...
mode_app(){
    ...
    [[ -n "$app_persistent_mode" ]] \
    && print_info "Persistent Mode (Enabled), Wait ($app_persistent_mode_wait_time sec)." \
    || print_info "Persistent Mode (Disabled)."
